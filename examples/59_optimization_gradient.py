...
def draw_antigrad_arrow_on_map(
    ...
    # Constant arrow length in parameter units
    arrow_len = 0.5
...
def render_map(
    ...
    # Anti-gradient arrow (next move direction), hidden when "optimal / stop"
...
def generate_frame(
    ...
    rmse_value, grad_b0, grad_b1 = rmse_and_gradients(current_b0, current_b1, features_scaled, target_scaled)
    grad_norm = float(np.hypot(float(grad_b0), float(grad_b1)))

    # Hide arrow when "optimal" (gradient small) OR on final "best model" frame
    ...
    fig.suptitle(
        f"{annotations.get_title()}. learning rate: {learning_rate:.2f}",
        fontsize=16,
        fontdict={"fontname": FONTNAME},
        va="top",
        x=0.5,
...
def show_optimal_b_search_gradient(
    ...
    # Final frame: mark as best model (last point) + NO arrow (handled by map_title_override)
    if len(path) > 0:
