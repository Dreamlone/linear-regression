...
def annotations_by_language(mode: str):
    if mode == "eng":
        ...
        y_label = "Price ($)"
        title = "Linear regression can predict price from the number of rooms"
        predicted_text = "so we can predict\nthis price"
        equation_text = r"$\text{price} = b_0 + b_1 \cdot \text{rooms}$"
        equation_text_optim = r"$\text{price} = 0 + 10000 \cdot \text{rooms}$"
        predicted_text_ext = "and this\none too"
    elif mode == "rus":
...
def plot_linear_regression(mode: str = "eng"):
    ...
    # Show plot
    plt.savefig(Path(get_plots_path(), f"4_plot_linear_regression_{mode}.svg"))
    plt.close()
