...
def annotations_by_language(mode: str):
    if mode == "eng":
        title = "Visualizing a sample as a frequency histogram (for variable V)"
        first_plot_title = "Dataset with two variables (U and V)"
        second_plot_title = "Overlay k bins\nand count observations"
        third_plot_title = "Frequency histogram"
        third_x = "Absolute frequency"
        fourth_plot_title = "Frequency histogram\n(vertical orientation)"
        fourth_x = "Relative frequency"
    elif mode == "rus":
...
def plot_hist_explanation(mode: str = "eng"):
    ...
    raw_svg_file = Path(get_plots_path(), f"extra_3_hist_explanation_{mode}.svg")
    final_plot = Path(get_plots_path(), f"extra_3_hist_explanation_{mode}.png")
