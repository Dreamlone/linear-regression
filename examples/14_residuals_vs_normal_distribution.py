...
def annotations_by_language(mode: str):
    if mode == "eng":
        ...
        title = (
            ...
            "Q-Q графики: нормальное распределение (оценка по остаткам) vs остатки"
        )
...
def plot_errors_plots(mode: str = "eng"):
    ...
    raw_svg_file = Path(get_plots_path(), f"14_datasets_residuals_qq_{mode}.svg")
    plt.savefig(raw_svg_file, bbox_inches="tight")
    plt.close()

    save_plot_according_to_template(
        raw_svg_file,
        Path(get_plots_path(), f"14_datasets_residuals_qq_{mode}.png"),
    )
