...
def plot_animation_about_mape_smape(mode: str = "eng", animation_duration: float = 2000):
    ...
    for x, y in produce_x_and_y_cases():
        ...
        raw_svg_file = Path(tmp_dir, f"animation_3_how_to_mape_smape_{mode}_{image_index}.svg")
        plt.savefig(raw_svg_file)
        plt.close()

        path_to_final_path = Path(tmp_dir, f"animation_3_how_to_calculate_mape_smape_{mode}_{image_index}.png")
    ...
    gif_path = Path(get_plots_path(), f"animation_3_how_to_calculate_mape_smape_{mode}.gif")
...
if __name__ == '__main__':
    ...
    plot_animation_about_mape_smape("eng")
