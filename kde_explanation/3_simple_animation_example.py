...
def compute_coordinates(number_of_frames: int, x_smalls, y_smalls):
    ...
    current_y = y_smalls.copy()

    for _ in range(number_of_frames):
        # Updating speed and coordinates
        v_y += g * dt
        current_y -= v_y * dt

        # Handling collisions with the floor
        touched_ground = current_y < ground_y
        current_y[touched_ground] = ground_y
        v_y[touched_ground] = 0

        xs.append(x_smalls.copy())
        ys.append(current_y.copy())
...
def animate(frame):
    ...
    ax.xaxis.set_ticks([])
    ax.plot([-2.25, 2.25], [0.5, 0.5], c='black', linewidth=4, zorder=1)
    ax.set_aspect('equal')
    if frame == 0:
        # First plot with initial circle
        circle = patches.Circle((0, 3), 0.5, edgecolor='r', facecolor='none', linewidth=2)
        ax.add_patch(circle)
    for i in range(len(xs[frame])):
        circle = patches.Circle((xs[frame][i], ys[frame][i]), PARTICLES_RADIUS,
                                edgecolor='r', facecolor='none', linewidth=0.5,
                                zorder=2)
        scat = ax.add_patch(circle)
    return scat,
...
if __name__ == '__main__':
    """ 
    Third attempt to create an animation of a circle “spilling out” like sand onto the ground
    Features of implementation:
        - "grains of sand" interact with each other when falling
        - the fall stops when "grains of sand" reach the surface
    """
    number_of_frames_in_animation = 50
    ...
    save_path = Path(get_kde_plots_path(), "3_simple_animation.gif")
